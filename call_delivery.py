...
class FirebaseCallPush:
    ...
    def send_invite(self, tokens: list[str], invite: dict[str, Any]) -> dict[str, Any]:
        ...
        return self._send_data(tokens, data, ttl_seconds=120, collapse_key=f"ombre_call_{data['inviteId']}")

    def send_proactive_message(self, tokens: list[str], item: dict[str, Any]) -> dict[str, Any]:
        data = {
            "type": "proactive_message",
            "messageId": str(item.get("id") or ""),
            "title": str(item.get("title") or "Zeta")[:60],
            "text": str(item.get("text") or "")[:1200],
            "createdAt": str(item.get("createdAt") or ""),
        }
        return self._send_data(tokens, data, ttl_seconds=21600, collapse_key=f"ombre_message_{data['messageId']}")
