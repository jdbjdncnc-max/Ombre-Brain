...
test("chat history migrates from localStorage to IndexedDB and removes the oversized legacy copy", async () => {
  const legacyMessages = [
    { id: "u1", role: "user", content: "旧记录", createdAt: "2026-08-12T10:00:00.000Z" }
  ];
  let removedKey = "";
  ...
  const nextMessages = [
    ...legacyMessages,
    { id: "a1", role: "assistant", content: "新记录", createdAt: "2026-08-12T10:00:01.000Z" }
  ];
  await store.save(nextMessages);
  assert.equal(await store.verify(nextMessages), true);
  assert.equal((await store.estimate()).kind, "indexeddb");
});

test("chat history still loads the legacy snapshot when IndexedDB is unavailable", async () => {
